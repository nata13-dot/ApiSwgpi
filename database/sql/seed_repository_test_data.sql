...
WHERE rd.nombre = 'Demo publico - Proyecto integrador';

INSERT IGNORE INTO repository_document_tag (repository_document_id, document_tag_id, created_at)
SELECT rd.id, dt.id, NOW()
FROM repository_documents rd
JOIN document_tags dt ON dt.nombre IN ('Prueba repositorio')
WHERE rd.nombre = 'Demo privado - Documento de evaluacion';

INSERT IGNORE INTO repository_document_tag (repository_document_id, document_tag_id, created_at)
SELECT rd.id, dt.id, NOW()
FROM repository_documents rd
JOIN document_tags dt ON dt.nombre IN ('Tesis 9no')
WHERE rd.nombre = 'Demo tesis general 9no';

INSERT IGNORE INTO repository_document_tag (repository_document_id, document_tag_id, created_at)
SELECT rd.id, dt.id, NOW()
FROM repository_documents rd
JOIN document_tags dt ON dt.nombre IN ('Tesis 9no', 'Residencias')
WHERE rd.nombre IN ('Demo tesis residencia 9no', 'Demo tesis residencia publicada');
